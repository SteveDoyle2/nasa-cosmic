/**********************************************************************
        vos.c 2.15 - Virtual operating system interface
...
9) This implementation of the vosopen/vosclose routines allows the same
   file to be open for both input and output which is normally not possible
   under unix.  The output may be written to without clobbering the file
   as long as the input file is opened first.
**********************************************************************/
...
int mode,nl,ns;
{
    ...
    if (mode==0) {
        ...
        if (name_unused) strcpy(tmpname,filename);
        fd=creat(tmpname,0644);
        if (fd==-1) {
            free(p);
            return(-1);
        }
        files[fd]=p;
        strcpy(files[fd]->filename,tmpname);
...
unsigned char *buffer;
{
    ...
    if (ns==files[fd]->ns && ss==1) {	/* write in blocks if possible */
        ns=nl*ns;
        nl=1;
    }
    for (j=sl;j<sl+nl;j++) {
...
int fd;
{
    char filename[80],newname[80],cmdbuf[80];
    int access_mode;
    strcpy(filename,files[fd]->filename);
    access_mode = files[fd]->access_mode;
    close(fd);
    free(files[fd]);
    files[fd]=0;
    if (strlen(filename)>4 && access_mode==1) {
        if (strcmp(filename+strlen(filename)-4,".tmp")==0) {
            strcpy(newname,filename);
            newname[strlen(filename)-4] = '\0';
            sprintf(cmdbuf,"mv %s %s",filename,newname);
            system(cmdbuf);
        }
    }
}
...
{
    long int i,j;
    if (ns==files[fd]->ns && ss==1) {	/* read in blocks if possible */
        ns=nl*ns;
        nl=1;
    }
    for (j=sl;j<sl+nl;j++) {
        lseek(fd,(j-1)*files[fd]->ns+ss-1+256*files[fd]->label_present,0);
        i=read(fd,buffer+(j-sl)*ns,ns);
        if (i!=ns) return(-1);
    }
    return(0);
}
...
{		/* problems when illegally-typed input is given.          */
    ...
    while (i!=-1 && i!=10) i=getchar();	/* remove chars beyond 79th in line. */
    buf[o] = '\0';
    if (strlen(buf)>0)
	while (isspace(buf[strlen(buf)-1])) buf[strlen(buf)-1]='\0';
    if (i==-1) strcpy(buf,"quit");
}


/************************************************************************/
/* vosdelete - remove specified file                                    */
/************************************************************************/
vosdelete(filename)
char *filename;
{
