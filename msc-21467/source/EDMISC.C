...
compile_region()
{
   ...
   add_router("emacs_region",90,region_fnd,
		NULL,region_getc,region_ungetc,NULL);

   /* COMPILE */

   get_compile("emacs_region","Emacs_region");
/*    fclose(trace); */
...
{ 
   ...
   add_router("emacs_file",90,buffer_fnd,
		NULL,buffer_getc,buffer_ungetc,NULL);

   /*   COMPILE   */

   get_compile("emacs_file","Emacs_buffer");
 ...
 **********************************************************/
get_compile(str1,str2)
char *str1;
char *str2;
{
   act_router(str1);		/* From CLIPS.C */
   act_router("cmp_router");
   LOAD_FLAG = TRUE;
   load_from_log_name(str2);    /* Load file _ From NPSR.C */
   deact_router(str1);		/* From CLIPS.C */
   deact_router("cmp_router");
   LOAD_FLAG = FALSE;
   del_router(str1);		/* From CLIPS.C */
}
...
showcpos(f, n)
{
        ...
        sprintf(prompt,"MicrEMACS Version %s   col: %d  line: %d of %d"
...
bkwrdrpl(f, n)
    ...
    if ((s = readpattern("Rev. replace all occrs of")) != TRUE)
        return (s);              
    pat2 = genalloc(strlen (pat) + 1);          /* Pat2 is first pattern */
    strcpy(pat2,pat);
    if((s = mlreply("Replace with: ",pat,NPAT)) == ABORT) {
        genfree(pat2, strlen(pat) + 1);
        return(s);
        }
...
bkwrdcr(f, n)
	...
	if (clp == curbp->b_linep)
     ...
     curwp->w_doto -= strlen(pat);
     update();
     buf = genalloc (40 + strlen(pat2));
     sprintf(buf,"No more occurence of [%s] in buffer",pat2);
     mlwrite(buf);
     genfree(buf, 40 + strlen(pat2));
     genfree(pat2, strlen(pat) + 1);
     return(TRUE);
   }
