...
killbuffer(f, n)
{
        ...
        if ((s=bclear(bp)) != TRUE)             /* Blow text away.      */
                return (s);
        genfree((char *) bp->b_linep, sizeof(LINE) + bp->b_linep->l_size);
...
register char   *bname;
{
        ...
        if (cflag != FALSE) {
                if ((bp=(BUFFER *)genalloc(sizeof(BUFFER))) == NULL)
                        return (NULL);
                if ((lp=lalloc(0)) == NULL) {
                        genfree((char *) bp, sizeof(BUFFER));
                        return (NULL);
                }
                bp->b_bufp  = bheadp;
                bheadp = bp;
...
register int    used;
{
        ...
        if ((lp = (LINE *) genalloc(sizeof(LINE)+size)) == NULL) {
...
register LINE   *lp;
{
        ...
        genfree((char *) lp, sizeof(LINE) + lp->l_size);
}
...
linsert(n, c)
{
        ...
        if (lp1->l_used+n > lp1->l_size) {      /* Hard: reallocate     */
                ...
                lp1->l_fp->l_bp = lp2;
                lp2->l_bp = lp1->l_bp;
                genfree((char *) lp1, sizeof(LINE) + lp1->l_size);
        } else {                                /* Easy: in place       */
...
ldelnewline()
{
        ...
        if (lp2->l_used <= lp1->l_size-lp1->l_used) {
                ...
                lp1->l_fp = lp2->l_fp;
                lp2->l_fp->l_bp = lp1;
                genfree((char *) lp2, sizeof(LINE) + lp2->l_size);
                return (TRUE);
        }
...
kinsert(c)
{
	...
	char far *nbufp;
        ...
        if (kused == ksize) {
                if ((nbufp= (char *) genalloc(ksize+KBLOCK)) == NULL)
                        return (FALSE);
                for (i=0; i<ksize; ++i)
                        nbufp[i] = kbufp[i];
                if (kbufp != NULL)
...
splitwind(f, n)
{
        ...
        if ((wp = (WINDOW *) genalloc(sizeof(WINDOW))) == NULL) {
...
vtinit()
{
    ...
    (*term.t_open)();
    vscreen = (VIDEO **) genalloc(term.t_nrow*sizeof(VIDEO *));

    if (vscreen == NULL)
        exit(1);

    pscreen = (VIDEO **) genalloc(term.t_nrow*sizeof(VIDEO *));
    ...
    for (i = 0; i < term.t_nrow; ++i)
        {
        vp = (VIDEO *) genalloc(sizeof(VIDEO)+term.t_ncol);
...
kill_video_buffers()
{
    ...
    for (i = 0; i < term.t_nrow; ++i) {
        genfree((char *) vscreen[i],sizeof(VIDEO)+term.t_ncol);
        genfree((char *) pscreen[i],sizeof(VIDEO)+term.t_ncol);
        }

    genfree((char *) vscreen, term.t_nrow*sizeof(VIDEO *));
    genfree((char *) pscreen, term.t_nrow*sizeof(VIDEO *));
 }
