	PROGRAM LA50
C *** LAST REVISED ON 15-MAR-1988 09:00:22.97
C *** SOURCE FILE: [DL.GRAPHICS.LONGLIB]LA50.FOR
C
C	CREATED:  DGL JULY 1987
C
C	THIS PROGRAM CONVERTS THE LONGLIB PRINTER HISTORY META FILE
C	INTO A DOT MATRIX PRINT FILE FOR AN LA50 PRINTER.
C	PROGRAM HAS ONLY HAD LIMITED TESTING.
C
...
C		2. USES IBCLR AND IBSET TO SET BITS IN A WORD.
...
C		   USED IN RSCOUT.
C		5. USES IAND AND IOR FOR BIT-WISE AND/OR FUNCTIONS.
C
	CHARACTER*80 NAME
C
...
C	OPEN OUTPUT FILE
C
	OPEN(UNIT=3,FILE='OUT.LIS',FORM='FORMATTED',RECL=1226,
     $		STATUS='NEW',ERR=399)
C
C	OPEN TEMPORARY WORKING FILE
C
	OPEN(UNIT=1,FILE='RASTER.TMP',FORM='UNFORMATTED',STATUS='NEW',
     $		ERR=199,ACCESS='SEQUENTIAL')
C     $		STATUS='SCRATCH',ERR=199,ACCESS='SEQUENTIAL')
C
...
C
	CHARACTER*(*) PAGE
	PARAMETER (PAGE=CHAR(27)//CHAR(47)//CHAR(27)//CHAR(80)
     $		//CHAR(81))
C
	WRITE (LU,1) PAGE
1	FORMAT('1',X,A5)
...
C
	...
	INTEGER ILT(10)
	DATA ILT/-1,255,3855,7295,13107,13119,15567,21845,22015,23485/
	INTEGER M(128)
...
C
	...
	IFIRST=0
C
C	INITIALIZE LAST LINE/COLOR SO THEY GET RESET TO NEW VALUES
C
	LSC=1
	LWIDE=1
	LLINE=0
...
C
C	DETERMINE LINE TYPE
C
	DO 1501,IL=1,10
		IF (M1.EQ.ILT(IL)) GOTO 1502
1501	CONTINUE
	IL=-1
1502	CONTINUE
C
...
C	DECODE COMMAND AND EXECUTE
C
	GOTO (10,20,30,40,55,60,10,80,80,50,10) IC
	GOTO 80
C
C	PEN DOWN MOVE
C
...
C
	...
	MM1=MMAP-1
C
C	FOR EACH NWORD ALONG PAGE LINES IN THE RASTER SCAN BUFFER
C
	DO 10 I=0,NMAP,NWORD
C
...
2		CONTINUE
C
C	COPY LINES INTO OUTPUT BUFFER
C
		NX=I*MMAP
		DO 15 J=0,MM1
			DO 5 JJ=0,NWORD-1
				...
				IBIT=MOD(NX+NJ+J,NBW)
				IOUT=IBITS(IMAP(IWOR),IBIT,1)
			IF (IOUT.NE.0) OUT(IWOR)=IBSET(OUT(J),JJ)
5			CONTINUE
15		CONTINUE	
C
...
6		CONTINUE
8		MLEN=JJ
		WRITE(LUO,25) ISPACE,(OUT(J),J=1,MLEN),MINUS
25		FORMAT(1226A1)
10	CONTINUE
	RETURN
	END
